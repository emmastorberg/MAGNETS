...
class BaseLitModel(pl.LightningModule):
    ...
    def training_step(
        ...
        x, y = batch
        t = torch.ones_like(y, device=x.device, dtype=torch.int64) * (x.size(2) - 1)
        prediction = self(x, t).squeeze(dim=-1)
        ...
        loss_mse = self._loss_fn(prediction, y)
        total_loss = loss_mse
...
class GATSM(BaseLitModel):
    ...
    def __init__(
        ...
        super().__init__(task, n_outputs, lr, weight_decay, target_scaling)
    ...
    def forward(
        ...
        x = x.permute(0, 2, 1)  # shape: [batch_size, n_steps, n_features]
