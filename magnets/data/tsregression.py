...
class TSRegressionDataset(Dataset):
    ...
    def __init__(self, path, problem, mode="train", scaling="none", target_scaling=1.0, target_log=False, subsampling=1, **kwargs):
        """
        ...
        filename = os.path.join(path, problem, problem + "_" + mode.upper() + ".ts")
        self.X, self.Y = load_from_tsfile_to_dataframe(filename)
        ...
        self.X = process_data(self.X, normalise=scaling, min_len=min_len)
        self.X = self.X.transpose(0, 2, 1)

        # subsample the data
        if subsampling > 1:
            self.X = self.X[:, :, ::subsampling]

        # pad to have length multiple of 8
        if self.X.shape[2] % 8 != 0:
            pad_len = 8 - self.X.shape[2] % 8
            self.X = np.pad(self.X, ((0, 0), (0, 0), (0, pad_len)), mode='constant')

        # standardize
        if problem == "PotassiumConcentration":
            self.X -= 0.66839757
            self.X /= 0.50665729
        elif problem == "WindTurbinePower":
            # remove outliers over 1000
            inlier = (self.X <= 1000).all(axis=(1, 2))
            self.Y = self.Y[inlier]
            self.X = self.X[inlier]
            self.X -= 19.6406018
            self.X /= 94.59481074
        elif problem == "HouseholdPowerConsumption1_nmv":
            self.X -= np.array([240.01554948,   4.71707354,   1.17389541,   1.45697135,
          5.93289067]).reshape(1, 5, 1)
            self.X /= np.array([3.31463421, 4.74635816, 6.32434319, 6.32590465, 8.21469954]).reshape(1, 5, 1)

            # remove outliers (optional)

        elif problem == "OccupancyDetectionLight":
            self.X -= np.array([19.57, 0.00, 652.80]).reshape(1, 3, 1)
            self.X /= np.array([5.15, 100.0, 347.88]).reshape(1, 3, 1)
        elif problem == "AppliancesEnergy":
            self.X -= np.array([21.74, 40.33, 20.48, 40.33, 22.28, 39.34, 20.85, 39.28, 19.61, 50.95, 8.24, 55.33, 20.26, 35.82, 21.97, 43.31, 19.47, 41.83, 7.74, 755.61, 79.83, 4.03, 38.72, 4.07]).reshape(1, 24, 1)
            self.X /= np.array([1.68, 4.02, 2.26, 4.13, 2.08, 3.38, 2.17, 4.50, 1.97, 8.81, 6.08, 31.55, 2.22, 5.20, 2.10, 5.27, 2.13, 4.24, 5.36, 7.21, 15.31, 2.47, 11.98, 4.18]).reshape(1, 24, 1)
        elif problem == "BenzeneConcentration_nmv":
            self.X -= np.array([1087.37, 952.76, 908.65, 1631.91, 963.69, 22.48, 42.75, 1.08]).reshape(1, 8, 1)
            self.X /= np.array([202.03, 247.91, 232.48, 360.94, 347.54, 7.75, 16.28, 0.30]).reshape(1, 8, 1)
        elif problem == "BeijingPM10Quality_nmv":
            self.X -= np.array([17.69, 51.59, 1243.85, 57.91, 14.28, 1009.95, 3.52, 0.06, 1.68]).reshape(1, 9, 1)
            self.X /= np.array([23.66, 35.33, 1140.67, 57.62, 11.06, 10.18, 13.35, 0.82, 1.24]).reshape(1, 9, 1)

        if target_log:
            self.Y = np.log(self.Y)

        self.Y /= target_scaling

    def __len__(self):
        return len(self.X)

    def __getitem__(self, idx):
        return torch.stack([torch.tensor(self.X[idx, j], dtype=torch.float32) for j in range(self.X.shape[1])]), torch.tensor(self.Y[idx], dtype=torch.float32)
